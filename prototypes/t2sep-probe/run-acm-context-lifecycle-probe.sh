...
[[ $confirmation == I_UNDERSTAND_EPHEMERAL_ACM_CONTEXT_LIFECYCLE_PROBE ]] ||
...
insmod "$module" apple_start_cpu_probe=1 apple_start_with_msi=1 \
  apple_send_control_nop=1 apple_probe_acm_context_lifecycle=1 \
  acm_context_confirmation=0x41434d4354584c46
rmmod t2sep_probe
trap - EXIT

log=$(journalctl -k --after-cursor "$before" --no-pager)
printf '%s\n' "$log"
python3 "$module_dir/verify-acm-context-lifecycle-log.py" <<<"$log" ||
  die "ACM context lifecycle transcript failed independent verification"
