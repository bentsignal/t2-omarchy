...
GOOD = "\n".join((
    ...
    line("OOL acknowledgement: request_opcode=2 tag=2 raw=07820200 00000000 00000000 00000000 decoded_endpoint=0 decoded_tag=2 decoded_opcode=130 decoded_target=7"),
    line("OOL registration request: opcode=3 tag=3 words=07030300 00000200 00004000 00000000"),
    line("OOL acknowledgement: request_opcode=3 tag=3 raw=07830300 00000000 00000000 00000000 decoded_endpoint=0 decoded_tag=3 decoded_opcode=131 decoded_target=7"),
...
class VerifyCredentialOolLogTests(unittest.TestCase):
    def test_accepts_exact_aks_transcript(self):
        self.assertEqual(verify.verify(GOOD, 7), ((130, 7), (131, 7)))

    def test_rejects_wrong_service_shape_status_and_order(self):
        mutations = (
            (GOOD, 10),
            (GOOD.replace("00004000", "0004b000", 1), 7),
            (GOOD.replace("raw=07820200 00000000", "raw=07820200 00000001"), 7),
