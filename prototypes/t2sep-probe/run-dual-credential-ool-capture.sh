...
[[ $confirmation == I_UNDERSTAND_NONSECRET_DUAL_CREDENTIAL_OOL_CAPTURE ]] ||
...
insmod "$module" apple_start_cpu_probe=1 apple_start_with_msi=1 \
  apple_send_control_nop=1 apple_capture_dual_credential_ool_acks=1 \
  dual_credential_ool_confirmation=0x4455414c4f4f4c41
rmmod t2sep_probe
trap - EXIT

log=$(journalctl -k --after-cursor "$before" --no-pager)
printf '%s\n' "$log"
python3 "$module_dir/verify-dual-credential-ool-log.py" <<<"$log" ||
  die "dual credential OOL transcript failed independent verification"
