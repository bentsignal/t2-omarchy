...
[[ $confirmation == I_UNDERSTAND_NONSECRET_AKS_STARTUP_ENVIRONMENT_PROBE ]] ||
...
insmod "$module" apple_start_cpu_probe=1 apple_start_with_msi=1 \
  apple_send_control_nop=1 apple_probe_aks_startup_environment=1 \
  aks_startup_environment_confirmation=0x414b53454e565052
rmmod t2sep_probe
trap - EXIT

log=$(journalctl -k --after-cursor "$before" --no-pager)
printf '%s\n' "$log"
python3 "$module_dir/verify-aks-startup-environment-log.py" <<<"$log" ||
  die "AKS startup-environment transcript failed independent verification"
