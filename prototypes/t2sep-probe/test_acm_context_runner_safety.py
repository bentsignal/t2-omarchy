...
SOURCE = Path(__file__).with_name(
    "run-acm-context-lifecycle-probe.sh").read_text()


class AcmContextRunnerSafetyTests(unittest.TestCase):
    def test_all_identity_and_confirmation_checks_precede_insmod(self):
        insmod = SOURCE.index('insmod "$module"')
        for marker in (
            "I_UNDERSTAND_EPHEMERAL_ACM_CONTEXT_LIFECYCLE_PROBE",
    ...
    def test_runner_is_single_lifecycle_and_verifies_cleanup(self):
        self.assertIn("apple_probe_acm_context_lifecycle=1", SOURCE)
        self.assertIn("verify-acm-context-lifecycle-log.py", SOURCE)
