...
[[ $confirmation == I_UNDERSTAND_NONSECRET_COMBINED_CREDENTIAL_STARTUP ]] ||
...
insmod "$module" apple_start_cpu_probe=1 apple_start_with_msi=1 \
  apple_send_control_nop=1 apple_probe_credential_startup=1 \
...
printf '%s\n' "$log"
python3 "$module_dir/verify-credential-startup-log.py" <<<"$log" ||
  die "combined credential-startup transcript failed independent verification"
