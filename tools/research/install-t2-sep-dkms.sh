...
trial=0
if [[ $# == 1 && $1 == --startup-trial ]]; then
  trial=1
elif [[ $# != 0 ]]; then
  echo 'Usage: install-t2-sep-dkms.sh [--startup-trial]' >&2; exit 1
fi
...
if [[ -e $destination || -L $destination ]]; then
  ...
  for file in Makefile t2_sep_transport.c t2_acm_lifecycle.h t2_aks_protocol.h t2_sep_transport_uapi.h; do
    cmp "$staging/$file" "$destination/$file"
  done
...
dkms_options=()
if (( trial )); then dkms_options+=(--force); fi
systemd-run --scope --quiet -p MemoryMax=1G -p MemorySwapMax=256M -p TasksMax=64 \
  dkms install -m t2-sep-transport -v "$version" -k "$kernel_release" "${dkms_options[@]}"
